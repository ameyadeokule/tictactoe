...
b2.addEventListener(
	'click',
	function () //adding eventlistener to RESTART button
	{
		restart()
	},
)
...
function restart() {
	//restart function to restart the game again
	_1.innerHTML = ''
	_2.innerHTML = ''
	_3.innerHTML = ''
	_4.innerHTML = ''
...
function disable() {
	//disable function to disable the button after the winner is displayed
...
document.querySelectorAll('button').forEach(
	(
		b, //accessing all the buttons i.e 9 buttons
	) => {
		b.addEventListener(
			'click',
			function () //adding the eventlistener to all those 9 buttons
			{
				if (sign == 'X' && b.innerHTML == '') {
					//checking the button is empty an if sign is X then adding the innerHtml to X
				...
				} else if (b.innerHTML == '' && sign == 'O') {
					//if the above condition is false this condition gets executed i.e if sign is O
...
function checkSign() {
	//function checksign
	if (sign == 'X') {
		//if sign is x then it will change to O and viceversa
		sign = 'O'
	} else {
		sign = 'X'
	}
...
function checkX() {
	//function to check if X is winner or not
	if (
...
function checkO() {
	//function to check if O is winner or not
	if (
...
function checkTie() {
	//function to check the game is tie or not
	if (
		_1.innerHTML != '' &&
		_2.innerHTML != '' &&
		_3.innerHTML != '' &&
...
function winner() {
	...
	if (checkX()) {
		...
		//checking the condition by calling the checkTie function
